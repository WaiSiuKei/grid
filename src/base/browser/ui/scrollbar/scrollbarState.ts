...
export class ScrollbarState {
  ...
  public clone(): ScrollbarState {
    let r = new ScrollbarState(this._scrollbarSize, this._oppositeScrollbarSize);
  ...
  private static _computeValues(oppositeScrollbarSize: number, visibleSize: number, scrollSize: number, scrollPosition: number) {
    const computedAvailableSize = Math.max(0, visibleSize - oppositeScrollbarSize);
    const computedRepresentableSize = Math.max(0, computedAvailableSize);
  ...
  private _refreshComputedValues(): void {
    const r = ScrollbarState._computeValues(this._oppositeScrollbarSize, this._visibleSize, this._scrollSize, this._scrollPosition);
  ...
  public getDesiredScrollPositionFromOffset(offset: number): number {
    ...
    let desiredSliderPosition = offset - this._computedSliderSize / 2;
